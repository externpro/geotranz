 ...
 *    03-18-08          Changed Mercator 'origin latitude' to 'latitude of true scale'
 */
 ...
 *  usng.h      - conversion between Geodetic and USNG coordinates
 ...
 *  gars.h      - conversion between Geodetic and GARS coordinates
 ...
 *  lambert_2.h - conversion between Geodetic and Lambert Conformal Conic (2 parallel) coordinates
...
typedef union Parameters
{
  ...
  Lambert_Conformal_Conic_2_Parameters  Lambert_Conformal_Conic_2;
...
typedef union Coordinate_Tuples
{
  ...
  Lambert_Conformal_Conic_2_Tuple   Lambert_Conformal_Conic_2;
...
{ /* Set_Defaults */
  ...
  strcpy(Default_Coordinates[GARS].GARS.string,"361HN37");
  ...
  Get_Lambert_2_Parameters(&a, &f, &origin_latitude, &central_meridian,
                           &std_parallel_1, &std_parallel_2, &false_easting, &false_northing);
  Default_Parameters[Lambert_Conformal_Conic_2].Lambert_Conformal_Conic_2.std_parallel_1 = std_parallel_1;
  Default_Parameters[Lambert_Conformal_Conic_2].Lambert_Conformal_Conic_2.std_parallel_2 = std_parallel_2;
  Default_Parameters[Lambert_Conformal_Conic_2].Lambert_Conformal_Conic_2.origin_latitude = origin_latitude;
  Default_Parameters[Lambert_Conformal_Conic_2].Lambert_Conformal_Conic_2.central_meridian = central_meridian;
  Default_Parameters[Lambert_Conformal_Conic_2].Lambert_Conformal_Conic_2.false_easting = false_easting;
  Default_Parameters[Lambert_Conformal_Conic_2].Lambert_Conformal_Conic_2.false_northing = false_northing;
  Default_Coordinates[Lambert_Conformal_Conic_2].Lambert_Conformal_Conic_2.easting = 0.0;
  Default_Coordinates[Lambert_Conformal_Conic_2].Lambert_Conformal_Conic_2.northing = 0.0;
  ...
  Get_Mercator_Parameters(&a, &f, &latitude_of_true_scale, &central_meridian,
                          &false_easting, &false_northing, &scale_factor);
  Default_Parameters[Mercator].Mercator.latitude_of_true_scale = latitude_of_true_scale;
  ...
  strcpy(Default_Coordinates[USNG].USNG.string,"31NEA0000000000");
...
{ /* Initialize_Engine */
  ...
  long temp_error_code = ENGINE_NO_ERROR;
  int i, j;

  if (Engine_Initialized)
    return (error_code);

  // Initialize ellipsoid table
  temp_error_code = Initialize_Ellipsoids();
  if(temp_error_code & ELLIPSE_FILE_OPEN_ERROR)
    error_code |= ENGINE_ELLIPSOID_ERROR;
  if(temp_error_code & ELLIPSE_TABLE_OVERFLOW_ERROR)
    error_code |= ENGINE_ELLIPSOID_OVERFLOW;
  if(temp_error_code & ELLIPSE_INITIALIZE_ERROR)
    error_code |= ENGINE_INVALID_CODE_ERROR;

  // Initialize datum table
  temp_error_code = Initialize_Datums();
  if(temp_error_code & DATUM_7PARAM_FILE_OPEN_ERROR || temp_error_code & DATUM_3PARAM_FILE_OPEN_ERROR)
    error_code |= ENGINE_DATUM_ERROR;
  if(temp_error_code & DATUM_7PARAM_FILE_PARSING_ERROR || temp_error_code & DATUM_3PARAM_FILE_PARSING_ERROR)
    error_code |= ENGINE_DATUM_FILE_PARSE_ERROR;
  if(temp_error_code & DATUM_7PARAM_OVERFLOW_ERROR || temp_error_code & DATUM_3PARAM_OVERFLOW_ERROR)
    error_code |= ENGINE_DATUM_OVERFLOW;

  // Initialize geoid tables
  temp_error_code = Initialize_Geoid();
  if(temp_error_code & GEOID_FILE_OPEN_ERROR)
    error_code |= ENGINE_GEOID_ERROR;
  if(temp_error_code & GEOID_INITIALIZE_ERROR)
    error_code |= ENGINE_GEOID_FILE_PARSE_ERROR;
  ...
  strcpy(Coordinate_System_Table[GARS].Name, "Global Area Reference System (GARS)");
  strcpy(Coordinate_System_Table[GARS].Code, "GA");
  ...
  strcpy(Coordinate_System_Table[USNG].Name, "United States National Grid (USNG)");
  strcpy(Coordinate_System_Table[USNG].Code, "US");
  ...
  strcpy(Coordinate_System_Table[Lambert_Conformal_Conic_2].Name, "Lambert Conformal Conic (2 parallel)");
  strcpy(Coordinate_System_Table[Lambert_Conformal_Conic_2].Code, "L2");
...
long Define_Datum ( Define_Datum_Type Datum_Type,
                    const char *Code,
                    const char *Name,
...
/*
 *   Datum_Type     : Specifies 3 parameter or 7 parameter datum    (input)
 ...
 *   Rotation_X     : X rotation to WGS84 in arc seconds            (input)
 *   Rotation_Y     : Y rotation to WGS84 in arc seconds            (input)
 *   Rotation_Z     : Z rotation to WGS84 in arc seconds            (input)
 *   Scale_Factor   : Scale factor                                  (input)
 *
 * The function Define_Datum creates a new local (3 or 7-parameter) datum with the
 ...
 * or a new version of the 3-param.dat or 7-param.dat file cannot be created, an error code
...
{ /* Define_Datum */
  ...
  {
    if( Datum_Type == Three_Param_Datum_Type )
    {
      temp_error = Create_3Param_Datum (Code, Name, Ellipsoid_Code,
    ...
    else if( Datum_Type == Seven_Param_Datum_Type )
    {
      temp_error = Create_7Param_Datum (Code, Name, Ellipsoid_Code,
                                        Delta_X, Delta_Y, Delta_Z, Rotation_X, Rotation_Y, Rotation_Z,
                                        Scale_Factor);
      if (temp_error & DATUM_INVALID_CODE_ERROR)
        error_code |= ENGINE_INVALID_CODE_ERROR;
      if (temp_error & DATUM_ROTATION_ERROR)
        error_code |= ENGINE_DATUM_ERROR;
      if (temp_error & DATUM_SCALE_ERROR)
        error_code |= ENGINE_DATUM_ERROR;
      if (temp_error & DATUM_7PARAM_OVERFLOW_ERROR)
        error_code |= ENGINE_DATUM_OVERFLOW;
      if (temp_error & DATUM_ELLIPSE_ERROR)
        error_code |= ENGINE_ELLIPSOID_ERROR;
      if (temp_error & DATUM_7PARAM_FILE_OPEN_ERROR)
        error_code |= ENGINE_DATUM_ERROR;
      if (temp_error == DATUM_NO_ERROR)
        Datum_Count (&Number_of_Datums);
    }
 ...
 *   Code           : 5-letter new datum code.                      (input)
 *
 * The function Remove_Datum deletes a user defined local (3 or 7-parameter) datum  
 * with the specified code.  If the datum table has not been initialized, or 
 * a new version of the 3-param.dat or 7-param.dat file cannot be created, an error code is
...
long Set_GARS_Coordinates
( const File_or_Interactive State,
  const Input_or_Output     Direction,
  const GARS_Tuple          coordinates)
/*
 *  The function Set_GARS_Coordinates sets the GARS coordinates
 *  for the specified state..
 *  State        : Indicates whether the coordinates are to be set for
 *                 interactive or file processing                      (input)
 *  Direction    : Indicates whether the coordinates are to be set for
 *                 input or output                                     (input)
 *  coordinates  : GARS coordinate string value to be set              (input)
 */
{ /* Set_GARS_Coordinates */
  ...
  if (!error_code)
  {
    if (CS_State[State][Direction].type != GARS)
      error_code |= ENGINE_INVALID_TYPE;
    else
      CS_State[State][Direction].coordinates.GARS = coordinates;
  }
  return ( error_code );
} /* Set_GARS_Coordinates */


long Get_GARS_Coordinates
( const File_or_Interactive State,
  const Input_or_Output     Direction,
  GARS_Tuple                *coordinates)
/*
 *  The function Get_GARS_Coordinates returns the GARS coordinates
 *  for the specified state.
 *  State        : Indicates whether the coordinates are to be returned for
 *                 interactive or file processing                      (input)
 *  Direction    : Indicates whether the coordinates are to be returned for
 *                 input or output                                     (input)
 *  coordinates  : GARS coordinate string value to be returned         (output)
 */
{ /* Get_GARS_Coordinates */
  ...
  if (!error_code)
  {
    if (CS_State[State][Direction].type != GARS)
      ...
      *coordinates = CS_State[State][Direction].coordinates.GARS;
  }
  return ( error_code );
} /* Get_GARS_Coordinates */
...
long Set_Lambert_Conformal_Conic_2_Params
( const File_or_Interactive State,
  const Input_or_Output     Direction,
  const Lambert_Conformal_Conic_2_Parameters parameters)
/*
 *  The function Set_Lambert_Conformal_Conic_2_Params sets the Lambert
 *  Conformal Conic (2 parallel) projection parameters for the specified state.
 *  State        : Indicates whether the parameters are to be set for
 *                 interactive or file processing                      (input)
 *  Direction    : Indicates whether the parameters are to be set for
 *                 input or output                                     (input)
 *  parameters   : Lambert Conformal Conic (2 parallel) projection parameters to be set
 *                                                                     (input)
 */
{ /* Set_Lambert_Conformal_Conic_2_Params */
  ...
  if (!error_code)
  {
    if (CS_State[State][Direction].type != Lambert_Conformal_Conic_2)
      error_code |= ENGINE_INVALID_TYPE;
    else
      CS_State[State][Direction].parameters.Lambert_Conformal_Conic_2 = parameters;
  }
  return (error_code);
} /* Set_Lambert_Conformal_Conic_2_Params */


long Get_Lambert_Conformal_Conic_2_Params
( const File_or_Interactive State,
  const Input_or_Output     Direction,
  Lambert_Conformal_Conic_2_Parameters *parameters)
/*
 *  The function Get_Lambert_Conformal_Conic_2_Params returns the Lambert
 *  Conformal Conic (2 parallel) projection parameters for the specified state.
 *  State        : Indicates whether the parameters are to be returned for
 *                 interactive or file processing                      (input)
 *  Direction    : Indicates whether the parameters are to be returned for
 *                 input or output                                     (input)
 *  parameters   : Lambert Conformal Conic (2 parallel) projection parameters to be returned
 *                                                                     (output)
 */
{ /* Get_Lambert_Conformal_Conic_2_Params */
  ...
  if (!error_code)
  {
    if (CS_State[State][Direction].type != Lambert_Conformal_Conic_2)
      ...
      *parameters = CS_State[State][Direction].parameters.Lambert_Conformal_Conic_2;
  }
  return ( error_code );
} /* Get_Lambert_Conformal_Conic_2_Params */


long Set_Lambert_Conformal_Conic_2_Coordinates
( const File_or_Interactive State,
  const Input_or_Output     Direction,
  const Lambert_Conformal_Conic_2_Tuple coordinates)
/*
 *  The function Set_Lambert_Conformal_Conic_2_Coordinates sets the Lambert Conformal
 *  Conic (2 parallel) projection coordinates for the specified state.
 *  State        : Indicates whether the coordinates are to be set for
 *                 interactive or file processing                      (input)
 *  Direction    : Indicates whether the coordinates are to be set for
 *                 input or output                                     (input)
 *  coordinates  : Lambert Conformal Conic (2 parallel) projection coordinates to be set
 *                                                                     (input)
 */
{ /* Set_Lambert_Conformal_Conic_2_Coordinates */
  ...
  if (!error_code)
  {
    if (CS_State[State][Direction].type != Lambert_Conformal_Conic_2)
      ...
      CS_State[State][Direction].coordinates.Lambert_Conformal_Conic_2 = coordinates;
  }
  return ( error_code );
} /* Set_Lambert_Conformal_Conic_2_Coordinates */


long Get_Lambert_Conformal_Conic_2_Coordinates
( const File_or_Interactive State,
  const Input_or_Output     Direction,
  Lambert_Conformal_Conic_2_Tuple *coordinates)
/*
 *  The function Get_Lambert_Conformal_Conic_2_Coordinates returns the Lambert
 *  Conformal Conic (2 parallel) projection coordinates for the specified state.
 *  State          : Indicates whether the coordinates are to be returned for
 *                   interactive or file processing                    (input)
 *  Direction      : Indicates whether the coordinates are to be returned for
 *                   input or output                                   (input)
 *  coordinates    : Lambert Conformal Conic (2 parallel) projection coordinates to be returned
 *                                                                     (output)
 */
{ /* Get_Lambert_Conformal_Conic_2_Coordinates */
  ...
  if (!error_code)
  {
    if (CS_State[State][Direction].type != Lambert_Conformal_Conic_2)
      ...
      *coordinates = CS_State[State][Direction].coordinates.Lambert_Conformal_Conic_2;
  }
  return ( error_code );
} /* Get_Lambert_Conformal_Conic_2_Coordinates */
...
long Set_USNG_Coordinates
( const File_or_Interactive State,
  const Input_or_Output     Direction,
  const USNG_Tuple          coordinates)
/*
 *  The function Set_USNG_Coordinates sets the USNG projection coordinates
 *  for the specified state.
 *  State        : Indicates whether the coordinates are to be set for
 *                 interactive or file processing                      (input)
 *  Direction    : Indicates whether the coordinates are to be set for
 *                 input or output                                     (input)
 *  coordinates  : USNG projection coordinates to be set               (input)
 */
{ /* Set_USNG_Coordinates */
  ...
  if (!error_code)
  {
    if (CS_State[State][Direction].type != USNG)
      error_code |= ENGINE_INVALID_TYPE;
    else
      CS_State[State][Direction].coordinates.USNG = coordinates;
  }
  return ( error_code );
} /* Set_USNG_Coordinates */


long Get_USNG_Coordinates
( const File_or_Interactive State,
  const Input_or_Output     Direction,
  USNG_Tuple                *coordinates)
/*
 *  The function Get_USNG_Coordinates returns the USNG projection coordinates
 *  for the specified state.
 *  State          : Indicates whether the coordinates are to be returned for
 *                   interactive or file processing                    (input)
 *  Direction      : Indicates whether the coordinates are to be returned for
 *                   input or output                                   (input)
 *  coordinates    : USNG projection coordinates to be returned        (output)
 */
{ /* Get_USNG_Coordinates */
  ...
  if (!error_code)
  {
    if (CS_State[State][Direction].type != USNG)
      ...
      *coordinates = CS_State[State][Direction].coordinates.USNG;
  }
  return ( error_code );
} /* Get_USNG_Coordinates */
...
{ /* Convert */
  ...
  if (!error_code)
  {
    ...
    if (input->datum_Index == output->datum_Index)
    {
      ...
      else if ((input->type == USNG) &&
               (output->type == UTM) && (output->parameters.UTM.override == 0))
      {
        temp_error = Set_USNG_Parameters(input_a,input_f,
                                         Input_Ellipsoid_Code);
        if (temp_error)
        {
          if (temp_error & USNG_A_ERROR)
            input->status |= ENGINE_A_ERROR;
          if (temp_error & USNG_INV_F_ERROR)
            input->status |= ENGINE_INV_F_ERROR;
        }
        else
        {
          temp_error = Convert_USNG_To_UTM(input->coordinates.USNG.string,
                                           &(output->coordinates.UTM.zone),
                                           &(output->coordinates.UTM.hemisphere),
                                           &(output->coordinates.UTM.easting),
                                           &(output->coordinates.UTM.northing));
          if (temp_error & USNG_STRING_ERROR)
            input->status = ENGINE_STRING_ERROR;
        }
        special = 1;
      }
      else if ((input->type == USNG) && (output->type == UPS))
      {
        temp_error = Set_USNG_Parameters(input_a,input_f,
                                         Input_Ellipsoid_Code);
        if (temp_error)
        ...
        {
          temp_error = Convert_USNG_To_UPS(input->coordinates.USNG.string,
                                           &(output->coordinates.UPS.hemisphere),
                                           &(output->coordinates.UPS.easting),
                                           &(output->coordinates.UPS.northing));
          if (temp_error & USNG_STRING_ERROR)
      ...
      else if ((input->type == UTM) && (output->type == USNG))
      {
        temp_error = ENGINE_NO_ERROR;
        temp_error = Set_USNG_Parameters(output_a,output_f,
                                         Output_Ellipsoid_Code);
        if (temp_error)
        {
          if (temp_error & USNG_A_ERROR)
        ...
        else
        {
          ...
          if (temp_precision > 5)
            temp_precision = 5;
          temp_error = Convert_UTM_To_USNG(coord.zone,
                                           coord.hemisphere,
                                           coord.easting,
                                           coord.northing,
                                           temp_precision,
                                           output->coordinates.USNG.string);
          if (temp_error & USNG_ZONE_ERROR)
            input->status |= ENGINE_ZONE_ERROR;
          if (temp_error & USNG_HEMISPHERE_ERROR)
            input->status |= ENGINE_HEMISPHERE_ERROR;
          if (temp_error & USNG_EASTING_ERROR)
            input->status |= ENGINE_EASTING_ERROR;
          if (temp_error & USNG_NORTHING_ERROR)
            input->status |= ENGINE_NORTHING_ERROR;
        }
        special = 1;
      }
      else if ((input->type == UPS) && (output->type == USNG))
      {
        temp_error = ENGINE_NO_ERROR;
        temp_error = Set_USNG_Parameters(output_a,output_f,
                                         Output_Ellipsoid_Code);
        ...
        else
        {
          ...
          if (temp_precision > 5)
            temp_precision = 5;
          temp_error = Convert_UPS_To_USNG(coord.hemisphere,
                                           coord.easting,coord.northing,
                                           temp_precision,
                                           output->coordinates.USNG.string);
          if (temp_error & USNG_HEMISPHERE_ERROR)
    ...
    else
    {
      ...
      case GARS:
        {
          temp_error = Convert_GARS_To_Geodetic(input->coordinates.GARS.string,
                                                &(Converted_Geodetic.latitude),
                                                &(Converted_Geodetic.longitude));
          if (temp_error & GARS_STR_ERROR)
            input->status |= ENGINE_STRING_ERROR;
          if (temp_error & GARS_STR_LON_ERROR)
            input->status |= ENGINE_STRING_ERROR;
          if (temp_error & GARS_STR_LAT_ERROR)
            input->status |= ENGINE_STRING_ERROR;
          if (temp_error & GARS_STR_15_MIN_ERROR)
            input->status |= ENGINE_STR_LON_MIN_ERROR;
          if (temp_error & GARS_STR_5_MIN_ERROR)
            input->status |= ENGINE_STR_LAT_MIN_ERROR;
          break;
        }
      case Gnomonic:
      ...
      case Lambert_Conformal_Conic_2:
        {
          Lambert_Conformal_Conic_2_Parameters param = input->parameters.Lambert_Conformal_Conic_2;
          temp_error = Set_Lambert_2_Parameters(input_a,input_f,
                                                param.origin_latitude,
                                                param.central_meridian,
                                                param.std_parallel_1,
                                                param.std_parallel_2,
          ...
          if (temp_error)
          {
            if (temp_error & LAMBERT_2_A_ERROR)
              input->status |= ENGINE_A_ERROR;
            if (temp_error & LAMBERT_2_INV_F_ERROR)
              input->status |= ENGINE_INV_F_ERROR;
            if (temp_error & LAMBERT_2_SECOND_STDP_ERROR)
              input->status |= ENGINE_SECOND_STDP_ERROR;
            if (temp_error & LAMBERT_2_FIRST_STDP_ERROR)
              input->status |= ENGINE_FIRST_STDP_ERROR;
            if (temp_error & LAMBERT_2_FIRST_SECOND_ERROR)
              input->status |= ENGINE_FIRST_SECOND_ERROR;
            if (temp_error & LAMBERT_2_HEMISPHERE_ERROR)
              input->status |= ENGINE_HEMISPHERE_ERROR;
            if (temp_error & LAMBERT_2_CENT_MER_ERROR)
              input->status |= ENGINE_CENT_MER_ERROR;
            if (temp_error & LAMBERT_2_ORIGIN_LAT_ERROR)
              input->status |= ENGINE_ORIGIN_LAT_ERROR;
            if (temp_error & LAMBERT_2_SCALE_FACTOR_ERROR)
              input->status |= ENGINE_SCALE_FACTOR_ERROR;
          }
          else
          {
            Lambert_Conformal_Conic_2_Tuple coord = input->coordinates.Lambert_Conformal_Conic_2;
            temp_error = Convert_Lambert_2_To_Geodetic(coord.easting,coord.northing,
                                                       &(Converted_Geodetic.latitude),
                                                       &(Converted_Geodetic.longitude));
      ...
      case Mercator:
        {
          ...
          if (temp_error)
          {
            ...
            if (temp_error & MERC_INV_F_ERROR)
              input->status |= ENGINE_INV_F_ERROR;
            if (temp_error & MERC_LAT_OF_TRUE_SCALE_ERROR)
              input->status |= ENGINE_ORIGIN_LAT_ERROR;
      ...
      case MGRS:
        {
          ...
          else
          {
            ...
            if (temp_error & MGRS_ZONE_ERROR)
              input->status |= ENGINE_ZONE_ERROR;
            if (temp_error & MGRS_LAT_WARNING)
              output->status |= ENGINE_LAT_WARNING;
          }
          break;
        }
      ...
      case Neys:
        {
          ...
          if (temp_error)
          {
            ...
            if (temp_error & NEYS_ORIGIN_LAT_ERROR)
              input->status |= ENGINE_ORIGIN_LAT_ERROR;
            if (temp_error & NEYS_SCALE_FACTOR_ERROR)
              input->status |= ENGINE_SCALE_FACTOR_ERROR;
          }
          else
          {
      ...
      case USNG:
        {
          ...
          {
            temp_error = Convert_USNG_To_Geodetic(input->coordinates.USNG.string,
                                                  &(Converted_Geodetic.latitude),
                                                  &(Converted_Geodetic.longitude));
            if (temp_error & USNG_STRING_ERROR)
              input->status = ENGINE_STRING_ERROR;
            ...
            if (temp_error & USNG_ZONE_ERROR)
              input->status |= ENGINE_ZONE_ERROR;
            if (temp_error & USNG_LAT_WARNING)
              output->status |= ENGINE_LAT_WARNING;
          }
          break;
        }
      ...
      if ((input->status) &&
          ...
          (input->status != ENGINE_LON_WARNING) && (input->status != ENGINE_LAT_WARNING))
        error_code |= ENGINE_INPUT_ERROR;
      if (input->status == ENGINE_LON_WARNING || input->status == ENGINE_LAT_WARNING)
        error_code |= ENGINE_INPUT_WARNING;
      /******************************/
      /* Datum Transformation Stage */
      /******************************/
      if (!(input->status) || input->status == ENGINE_LON_WARNING || input->status == ENGINE_LAT_WARNING)
      { /* Datum transformation */
        ...
        { /* Copy coordinate tuple */
          ...
          if(input->datum_Index != WGS84_datum_index && output->datum_Index != WGS84_datum_index)
          {
        ...
        { /* Shift to WGS84, apply geoid correction, shift to output datum */
          if (input->datum_Index != WGS84_datum_index)
          {
            ...
            {
              case Geoid_or_MSL_Height:
              case MSL_EGM96_VG_NS_Height:
              case MSL_EGM84_10D_BL_Height:
              case MSL_EGM84_10D_NS_Height:
          ...
          if(input_height_type != output_height_type)
          {
            ...
            {
              case Geoid_or_MSL_Height:
                Convert_Geoid_To_Ellipsoid_Height(WGS84_Geodetic.latitude, WGS84_Geodetic.longitude,
                                                  WGS84_Geodetic.height, &temp_height);
                break;
              case MSL_EGM96_VG_NS_Height:
                Convert_MSL_EGM96_VG_NS_To_Ellipsoid_Height(WGS84_Geodetic.latitude, WGS84_Geodetic.longitude,
                                                            WGS84_Geodetic.height, &temp_height);
                break;
              case MSL_EGM84_10D_BL_Height:
                Convert_MSL_EGM84_10D_BL_To_Ellipsoid_Height(WGS84_Geodetic.latitude, WGS84_Geodetic.longitude,
                                                             WGS84_Geodetic.height, &temp_height);
                break;
              case MSL_EGM84_10D_NS_Height:
                Convert_MSL_EGM84_10D_NS_To_Ellipsoid_Height(WGS84_Geodetic.latitude, WGS84_Geodetic.longitude,
                                                             WGS84_Geodetic.height, &temp_height);
                break;
              case Ellipsoid_Height:
              default:
                temp_height = WGS84_Geodetic.height;
                break;
            }
            ...
            {
              case Geoid_or_MSL_Height:
                Convert_Ellipsoid_To_Geoid_Height(WGS84_Geodetic.latitude, WGS84_Geodetic.longitude,
                                                  temp_height, &Corrected_Height);
                break;
              case MSL_EGM96_VG_NS_Height:
                Convert_Ellipsoid_To_MSL_EGM96_VG_NS_Height(WGS84_Geodetic.latitude, WGS84_Geodetic.longitude,
                                                            temp_height, &Corrected_Height);
                break;
              case MSL_EGM84_10D_BL_Height:
                Convert_Ellipsoid_To_MSL_EGM84_10D_BL_Height(WGS84_Geodetic.latitude, WGS84_Geodetic.longitude,
                                                             temp_height, &Corrected_Height);
                break;
              case MSL_EGM84_10D_NS_Height:
                Convert_Ellipsoid_To_MSL_EGM84_10D_NS_Height(WGS84_Geodetic.latitude, WGS84_Geodetic.longitude,
                                                             temp_height, &Corrected_Height);
                break;
              case Ellipsoid_Height:
              default:
                Corrected_Height = temp_height;
                break;
            }
      ...
      if (error_code != ENGINE_INPUT_ERROR)
      {
        ...
        case GARS:
          {
            ...
            if (temp_precision > 5)
              temp_precision = 5;
            temp_error = Convert_Geodetic_To_GARS(Shifted_Geodetic.latitude,
                                                  Shifted_Geodetic.longitude,
                                                  temp_precision,
                                                  output->coordinates.GARS.string);
            if (temp_error & GARS_LAT_ERROR)
              output->status |= ENGINE_LAT_ERROR;
            if (temp_error & GARS_LON_ERROR)
              output->status |= ENGINE_LON_ERROR;
            break;
          }
        case Gnomonic:
        ...
        case Lambert_Conformal_Conic_2:
          {
            Lambert_Conformal_Conic_2_Parameters param = output->parameters.Lambert_Conformal_Conic_2;
            temp_error = Set_Lambert_2_Parameters(output_a,output_f,
                                                  param.origin_latitude,
                                                  param.central_meridian,
                                                  param.std_parallel_1,
                                                  param.std_parallel_2,
            ...
            else
            {
              ...
              temp_error = Convert_Geodetic_To_Lambert_2(Shifted_Geodetic.latitude,
                                                         Shifted_Geodetic.longitude,
                                                         &(coord.easting),
                                                         &(coord.northing));
              output->coordinates.Lambert_Conformal_Conic_2 = coord;
              if (temp_error & LAMBERT_2_LAT_ERROR)
                output->status |= ENGINE_LAT_ERROR;
              if (temp_error & LAMBERT_2_LON_ERROR)
                output->status |= ENGINE_LON_ERROR;
              if (temp_error & LAMBERT_2_A_ERROR)
                output->status |= ENGINE_A_ERROR;
              if (temp_error & LAMBERT_2_INV_F_ERROR)
        ...
        case MGRS:
          {
            ...
            else
            {
              ...
              if (temp_error & MGRS_LON_ERROR)
                output->status |= ENGINE_LON_ERROR;
              if (temp_error & MGRS_PRECISION_ERROR)
                output->status |= ENGINE_PRECISION_ERROR;
              if (temp_error & MGRS_A_ERROR)
                output->status |= ENGINE_A_ERROR;
              if (temp_error & MGRS_INV_F_ERROR)
                output->status |= ENGINE_INV_F_ERROR;
        ...
        case USNG:
          {
            ...
            else
            {
              ...
              if (temp_precision > 5)
                temp_precision = 5;
              temp_error = Convert_Geodetic_To_USNG(Shifted_Geodetic.latitude,
                                                    Shifted_Geodetic.longitude,
                                                    temp_precision,
                                                    output->coordinates.USNG.string);

              if (temp_error & USNG_LAT_ERROR)
                output->status |= ENGINE_LAT_ERROR;
              if (temp_error & USNG_LON_ERROR)
                output->status |= ENGINE_LON_ERROR;
              if (temp_error & USNG_PRECISION_ERROR)
                output->status |= ENGINE_PRECISION_ERROR;
              if (temp_error & USNG_A_ERROR)
                output->status |= ENGINE_A_ERROR;
              if (temp_error & USNG_INV_F_ERROR)
        ...
        /* switch (output->type) */
        tempStatus = output->status - (output->status & ENGINE_DATUM_WARNING) - (output->status & ENGINE_LON_WARNING) - (output->status & ENGINE_LAT_WARNING);
        if( tempStatus )
          error_code |= ENGINE_OUTPUT_ERROR;
        if (output->status & ENGINE_LON_WARNING || output->status & ENGINE_LAT_WARNING || output->status & ENGINE_DATUM_WARNING)
          error_code |= ENGINE_OUTPUT_WARNING;
        if ((input->status) || (output->status))
 ...
 *  of the most recently converted location
 *  State  : Indicates whether the datum is to be used for interactive
 *           or file processing                                    (input)
...
{ /* Get_Conversion_Status_String */
  ...
  if (!error_code)
  {
    ...
    if (Error_Code & ENGINE_LAT_WARNING)
    {
      strcat(String,"Latitude band boundary cuts across 100km square");
      strcat(String,Separator);
    }
    ...
    if (Error_Code & ENGINE_PRECISION_ERROR)
    {
      strcat(String,"Precision must be between 0 and 5");
      strcat(String,Separator);
    }
    ...
    if (Error_Code & ENGINE_STRING_ERROR)
    {
      switch (System)
      {
        ...
        case GARS:
        {
          strcat(String,"Invalid GARS String");
          break;
        }
        case MGRS:
        {
        ...
        case USNG:
        {
          strcat(String,"Invalid USNG String");
          break;
        }
      }
      strcat(String,Separator);
    ...
    if (Error_Code & ENGINE_ELLIPSOID_CODE_ERROR)
    {
      switch (System)
      {
        ...
        case USNG:
        {
          strcat(String,"Ellipsoid must be WGS84, GRS80 or Clarke 1866");
          break;
        }
      }
      strcat(String,Separator);
...
{ /* Get_Return_Code_String */
  ...
  if (Error_Code & ENGINE_NOT_INITIALIZED)
  {
    strcat(String,"Engine not initialized");
    strcat(String, Separator);
  }
...
/*  if (Error_Code & ENGINE_DATUM_ROTATION_ERROR)
  {
    strcat(String,"Standard error values must be between -0.001 and 0.001");
    strcat(String,Separator);
  }
  if (Error_Code & ENGINE_DATUM_SCALE_ERROR)
  {
    strcat(String,"Standard error values must be between -60.0 and 60.0");
    strcat(String,Separator);
  }*/
