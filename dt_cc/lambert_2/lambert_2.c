...
static double Lambert_Delta_Easting = 400000000.0;
static double Lambert_Delta_Northing = 400000000.0;
 ...
 */
  ...
  double const_value;
  double inv_f = 1 / f;
  long lambert1_error_Code = LAMBERT_2_NO_ERROR;
  ...
  { /* no errors */
    ...
    lambert1_error_Code = Set_Lambert_1_Parameters(Lambert_a, Lambert_f, Lambert_lat0, Lambert_Origin_Long, Lambert_False_Easting, Lambert_false_northing, Lambert_k0);

    if(lambert1_error_Code)
    {
      if (lambert1_error_Code & LAMBERT_1_A_ERROR)
        Error_Code |= LAMBERT_2_A_ERROR;
      if (lambert1_error_Code & LAMBERT_1_INV_F_ERROR)
        Error_Code |= LAMBERT_2_INV_F_ERROR;
      if (lambert1_error_Code & LAMBERT_1_ORIGIN_LAT_ERROR)
        Error_Code |= LAMBERT_2_ORIGIN_LAT_ERROR;
      if (lambert1_error_Code & LAMBERT_1_CENT_MER_ERROR)
        Error_Code |= LAMBERT_2_CENT_MER_ERROR;
      if (lambert1_error_Code & LAMBERT_1_SCALE_FACTOR_ERROR)
        Error_Code |= LAMBERT_2_SCALE_FACTOR_ERROR;
    }
  }
  return (Error_Code);
 ...
 */
  ...
  { /* no errors */
    ...
    if(!lambert1_error_Code)
    {
      lambert1_error_Code = Convert_Geodetic_To_Lambert_1(Latitude, Longitude, Easting, Northing);
      if(lambert1_error_Code)
      {
        if (lambert1_error_Code & LAMBERT_1_LAT_ERROR)
          Error_Code |= LAMBERT_2_LAT_ERROR;
        if (lambert1_error_Code & LAMBERT_1_LON_ERROR)
          Error_Code |= LAMBERT_2_LON_ERROR;
      }
    }
    else
 ...
 */
  ...
  { /* no errors */
    ...
    if(!lambert1_error_Code)
    {
      lambert1_error_Code = Convert_Lambert_1_To_Geodetic(Easting, Northing, Latitude, Longitude);
      if(lambert1_error_Code)
      {
        if (lambert1_error_Code & LAMBERT_1_EASTING_ERROR)
          Error_Code |= LAMBERT_2_EASTING_ERROR;
        if (lambert1_error_Code & LAMBERT_1_NORTHING_ERROR)
          Error_Code |= LAMBERT_2_NORTHING_ERROR;
      }
    }
    else
