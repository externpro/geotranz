...
static double Neys_Delta_Easting = 400000000.0;
static double Neys_Delta_Northing = 400000000.0;
 ...
 */
  ...
  double inv_f = 1 / f;
  long lambert2_error_code = NEYS_NO_ERROR;
  ...
  { /* no errors */
    ...
    lambert2_error_code = Set_Lambert_2_Parameters(Neys_a, Neys_f, Neys_Origin_Lat, Neys_Origin_Long,
                                                   Neys_Std_Parallel_1, Neys_Std_Parallel_2,
                                                   Neys_False_Easting, Neys_False_Northing);
    if(lambert2_error_code)
    {
      if (lambert2_error_code & LAMBERT_2_A_ERROR)
        Error_Code |= NEYS_A_ERROR;
      if (lambert2_error_code & LAMBERT_2_INV_F_ERROR)
        Error_Code |= NEYS_INV_F_ERROR;
      if (lambert2_error_code & LAMBERT_2_ORIGIN_LAT_ERROR)
        Error_Code |= NEYS_ORIGIN_LAT_ERROR;
      if (lambert2_error_code & LAMBERT_2_CENT_MER_ERROR)
        Error_Code |= NEYS_CENT_MER_ERROR;
      if (lambert2_error_code & LAMBERT_2_SCALE_FACTOR_ERROR)
        Error_Code |= NEYS_SCALE_FACTOR_ERROR;
    }
  }
  return (Error_Code);
 ...
 */
  ...
  { /* no errors */
    ...
    lambert2_error_code = Convert_Geodetic_To_Lambert_2(Latitude, Longitude, Easting, Northing);

    if (lambert2_error_code & LAMBERT_2_LAT_ERROR)
      Error_Code |= NEYS_LAT_ERROR;
    if (lambert2_error_code & LAMBERT_2_LON_ERROR)
      Error_Code |= NEYS_LON_ERROR;
 ...
 */
  ...
  { /* no errors */
    ...
    lambert2_error_code = Convert_Lambert_2_To_Geodetic(Easting, Northing, Latitude, Longitude);

    if (lambert2_error_code & LAMBERT_2_EASTING_ERROR)
      Error_Code |= NEYS_EASTING_ERROR;
    if (lambert2_error_code & LAMBERT_2_NORTHING_ERROR)
      Error_Code |= NEYS_NORTHING_ERROR;
