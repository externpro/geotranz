 ...
 *    by one standard parallel and specified scale true along that parallel.  
 *
 * ERROR HANDLING
 *
 ...
 *                                      (-180 to 360 degrees)
 *		   LAMBERT_1_SCALE_FACTOR_ERROR : Scale factor greater than minimum scale
 *                                      factor (1.0e-9)
...
#define MIN_SCALE_FACTOR  1.0e-9
#define ONE_SECOND        4.89e-6
...
static double Lambert_1_Origin_Lat = (45.0 * PI / 180.0); /* Latitude of origin in radians */
 ...
 */
  ...
  if (((Origin_Latitude < -MAX_LAT) || (Origin_Latitude > MAX_LAT)) ||
       (Origin_Latitude > -ONE_SECOND) && (Origin_Latitude < ONE_SECOND))
