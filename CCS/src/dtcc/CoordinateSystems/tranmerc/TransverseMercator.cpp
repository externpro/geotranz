...
TransverseMercator::TransverseMercator( double ellipsoidSemiMajorAxis, double ellipsoidFlattening, double centralMeridian, double latitudeOfTrueScale, double falseEasting, double falseNorthing, double scaleFactor ) :
  CoordinateSystem(),
 ...
 */
  ...
  try
  {
    GeodeticCoordinates gcDeltaNorthing( CoordinateType::geodetic, MAX_DELTA_LONG + TranMerc_Origin_Long, MAX_LAT );
    ...
    GeodeticCoordinates gcDeltaEasting( CoordinateType::geodetic, MAX_DELTA_LONG + TranMerc_Origin_Long, 0 );
 ...
 */
  ...
  return new MapProjection5Parameters( CoordinateType::transverseMercator, TranMerc_Origin_Long, TranMerc_Origin_Lat, TranMerc_Scale_Factor, TranMerc_False_Easting, TranMerc_False_Northing );
}
 ...
 */
  ...
  double northing = TranMerc_False_Northing + t1 + pow(dlam, 2.e0) * t2
  ...
  double easting = TranMerc_False_Easting + dlam * t6 + pow(dlam,3.e0) * t7 
             + pow(dlam,5.e0) * t8 + pow(dlam,7.e0) * t9;

  return new MapProjectionCoordinates( CoordinateType::transverseMercator, errorStatus, easting, northing );
}
...
double TransverseMercator::sphtmd( double latitude )
{
...
double TransverseMercator::sphsn( double  latitude ) 
{
  return semiMajorAxis / sqrt( 1.e0 - TranMerc_es * pow(sin(latitude), 2.0));
}


double TransverseMercator::sphsr( double latitude ) 
{
